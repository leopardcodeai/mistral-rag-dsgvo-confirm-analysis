...
print("Installing dependencies...")subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", 
    "transformers", "datasets", "boto3", "accelerate", "torch", "huggingface_hub"])# Hugging Face Login (User wants login nehmen!)#
print("\n" + "="*50)#
...
model = AutoModelForCausalLM.from_pretrained(
    model_name,    torch_dtype=torch.bfloat16,  # T4 supports bf16    device_map="auto")#
...
def format_instruction(example):    # Qwen2.5 format: <|im_start|>system/user/assistant...<|im_end|>    return {"text": f"<|im_start|>user\n{example['prompt']}<|im_end|>\n<|im_start|>assitant\n{example['completion']}<|im_end|>"}#
...
training_args = TrainingArguments(
    output_dir="./rsi-qwen3b-finetuned",    num_train_epochs=10,              # 10 epochs for 12 records    per_device_train_batch_size=1,    gradient_accumulation_steps=4,    learning_rate=5e-4,    bf16=True,                       # T4: bfloat16    save_steps=10,    save_total_limit=2,    logging_steps=1,    report_to="none"                  # No WandB in Colab)#

from transformers import DataCollatorForLanguageModeling#
collator = DataCollatorForLanguageModeling(tokenizer, mlm=False)#

trainer = Trainer(
    model=model,    args=training_args,    train_dataset=tokenized_dataset,    data_collator=collator)## ==========================================# 7. START TRAINING#
=========================================trainer.train()#
